...
export async function previewRequest(path: string, options: { method?: string; body?: unknown } = {}): Promise<unknown> {
  ...
  if (p === "/ai/chat") return { report: /report/i.test(body.message) ? { disease: "Sample", summary: "UI preview only. No medical assessment performed.", symptoms: [], causes: [], treatments: [], precautions: [] } : undefined, response: "This is a UI preview response. Your message would appear here with guidance from Viruj AI once the backend is connected. No medical assessment was performed.", sessionId: body.sessionId || String(Date.now()), suggestedTitle: body.message.slice(0, 35) };
