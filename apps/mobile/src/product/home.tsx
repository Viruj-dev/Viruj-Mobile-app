...
export function Home({ navigate }: { navigate: Navigate }) {
  ...
  return <Screen title={`Hello, ${session?.user.name.split(" ")[0] || "there"}`} subtitle="VIRUJ HEALTH" right={<Pressable style={s.iconButton} accessibilityRole="button" accessibilityLabel="Notifications" onPress={() => navigate({ name: "notifications" })}><Glyph name="notifications-outline" color={colors.ink} /></Pressable>}>
    ...
    <View style={{ backgroundColor: colors.deep, borderRadius: 28, padding: 24, gap: 22 }}><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><Text style={{ color: "#FECACA", fontSize: 12, letterSpacing: 1.5 }}>FIND CARE</Text><Glyph name="medical-outline" color="#FECACA" size={32} /></View><Text style={{ color: "white", fontSize: 32, lineHeight: 38, fontWeight: "600", letterSpacing: -1 }}>Find a doctor.</Text><Pressable accessibilityRole="button" onPress={() => navigate({ name: "care", kind: "doctors" })} style={{ flexDirection: "row", gap: 10, alignItems: "center", minHeight: 48 }}><Text style={{ color: "white", fontWeight: "600", fontSize: 15 }}>Explore doctors</Text><Glyph name="arrow-forward" color="white" size={20} /></Pressable></View>
