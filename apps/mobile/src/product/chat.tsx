...
export function Chat() {
  ...
  async function send() {
    ...
    try {
      const result = await api.request<{ response: string; sessionId: string; suggestedTitle?: string }>("/ai/chat", { method: "POST", body: { message: prompt, history: messages.slice(-20), sessionId } });
      const updated: Message[] = [...messages, { sender: "user", text: prompt }, { sender: "ai", text: result.response }];
      setMessages(updated); setText(""); setSessionId(result.sessionId); const nextTitle = result.suggestedTitle || title; setTitle(nextTitle);
  ...
  return <Screen title="Ask Viruj" subtitle="AI ASSISTANT"><View style={{ flexDirection: "row", gap: 10 }}><Button title="History" secondary disabled={busy} onPress={() => setHistory(true)} /><Button title="New chat" secondary disabled={busy} onPress={() => { setMessages([]); setSessionId(undefined); setTitle("New conversation"); setError(""); }} /></View><Body>AI guidance can be inaccurate. It does not replace a clinician.</Body>{messages.length === 0 && <Empty icon="sparkles-outline" title="What’s on your mind?" detail="Ask a health question." />}{messages.map((message, index) => <View key={index} style={{ backgroundColor: message.sender === "user" ? colors.deep : "white", padding: 18, borderRadius: 20, alignSelf: message.sender === "user" ? "flex-end" : "stretch", maxWidth: "100%", gap: 8 }}><Text style={{ color: message.sender === "user" ? "#D0F2E8" : colors.deep, fontWeight: "700", fontSize: 12 }}>{message.sender === "user" ? "YOU" : "VIRUJ AI"}</Text><Text selectable style={{ color: message.sender === "user" ? "white" : colors.ink, fontSize: 16, lineHeight: 25 }}>{message.text}</Text></View>)}<ErrorText message={error || saveError} /><Field label="Message" placeholder="Ask a question" value={text} onChangeText={setText} multiline maxLength={5000} /><Button title="Send" icon="arrow-up" busy={busy} disabled={!text.trim()} onPress={() => void send()} /></Screen>;
}
