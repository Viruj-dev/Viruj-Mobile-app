...
export function Care({ kind, navigate, back }: { kind: string; navigate: Navigate; back(): void }) {
  const [search, setSearch] = useState(""); const [query, setQuery] = useState(""); const [page, setPage] = useState(1);
  const result = useResource<{ data: CareItem[]; pagination?: { totalPages: number } }>(`/${kind}?page=${page}&limit=12&search=${encodeURIComponent(query)}`);
  const items = (result.data?.data || []).filter(item => kind !== "pathlabs" || `${item.name} ${item.city} ${item.area}`.toLowerCase().includes(query.toLowerCase()));
  return <Screen title={kind === "pathlabs" ? "Labs" : kind === "hospitals" ? "Hospitals" : "Doctors"} back={back}>
