...
export async function signOutFromProviders() {
  ...
  const [{ GoogleOneTapSignIn }, { LoginManager }] = await Promise.all([
    import("react-native-nitro-google-signin"), import("react-native-fbsdk-next"),
  ]);
  await Promise.allSettled([GoogleOneTapSignIn.signOut(), Promise.resolve(LoginManager.logOut())]);
}
